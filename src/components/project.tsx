...
export const Project: React.FC<ProjectProps> = ({ name, tagline, tags, bgImage, year, backgroundPositionY, children, blogurl, highlighted, ...props }) => {
    ...
    return (
        ...
        <div
            className={`flex flex-col overflow-hidden relative gap-y-2 w-full  ${highlighted ? 'project-highlighted' : 'shadow-lg'}`}
        >
            <div className="bg-neutral-900 border border-neutral-700 rounded-lg overflow-hidden w-full">
                ...
                <div className="absolute top-1 right-1 flex flex-row gap-x-1 ">
                    {
                        ...
                        <div className="p-1 bg-white/10 hover:bg-white/20 active:bg-black/20 shadow-xl backdrop-blur-[10px] rounded-full">
                            <SocialIcon website={props.steamHref}>
                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" className="bi bi-steam w-7 h-7" viewBox="0 0 16 16">
                ...
                <div className="px-5 mt-3 w-full ">
                    ...
                    <div className="pb-1 mt-1 mb-3">
                        {blogurl &&
                            <a className='text-xs flex gap-x-3 w-fit text-gray-200 hover:text-gray-50 mt-2' href={`https://alexeckardt.ca${blogurl}`}>Related Blog Post <ArrowRight width={14} height={14} /></a>
                        }
                    </div>
                </div>
            </div>
